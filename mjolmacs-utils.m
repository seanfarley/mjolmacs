...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"(modifiers %@, key: %@)", mods_str, rev_key_map[_key]];
}
...
NSArray *emacs_parse_keys(emacs_env *env, emacs_value ekb) {
  NSNumber *control = [NSNumber numberWithUnsignedLong:NSEventModifierFlagControl];
  NSNumber *meta = [NSNumber numberWithUnsignedLong:NSEventModifierFlagOption];
  NSNumber *command = [NSNumber numberWithUnsignedLong:NSEventModifierFlagCommand];
  NSNumber *shift = [NSNumber numberWithUnsignedLong:NSEventModifierFlagShift];

  NSDictionary *modifier_map = @{
    @"C" : control,
    // @"H" : @"Hyper",
    @"M" : meta,
    @"s" : command,
